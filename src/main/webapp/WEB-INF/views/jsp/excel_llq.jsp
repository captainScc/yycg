    ...
    <style>
        *{
        ...
        .dd_title{
            width: 100%;
            height:100px;
            background: url(${pageContext.request.contextPath }/images/logo/llq.png) no-repeat center center;
        }
        .dd_a{
            width: 100%;
            height:80px;
...
<%--</header>--%>
<div class="dd">
    <div class="dd_title"></div>
    <div class="dd_a">后台管理员</div>
</div>
<div class="container">
        ...
        <c:if test="${totalPage <= 10}">
            <c:forEach begin="1" end="${totalPage}" step="1" var="i">
                <c:if test="${pageIndex == i}">
                    <a style="background:#0E101F;color:#fff" href="${pageContext.request.contextPath }/selectData.do?pageIndex=${i}&startTime=${startTime}&endTime=${endTime}&name=${name}">${i}</a>
                </c:if>
                <c:if test="${pageIndex != i}">
